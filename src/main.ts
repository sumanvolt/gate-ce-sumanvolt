/* =========================================================
   ...
   main.ts — typed app state, rendering, countdown, chart, undo, PWA
   ========================================================= */
...
(function () {
  ...
  interface AppState {
    syllabus: Record<string, SubtopicProgress>;
    mocks: MockTest[];
  ...
  const GATE_TARGET_DATE = new Date('2027-02-06T09:30:00+05:30');
  ...
  const CHECK_KINDS = ['theory', 'pyq', 'notes', 'revision'] as const;
  const CHECK_LABELS: Record<string, string> = { theory: 'THEORY', pyq: 'PYQ', notes: 'NOTES', revision: 'REV' };

  function slugify(subjectId: string, subtopicIndex: number): string {
  ...
  function buildDefaultState(): AppState {
    const syllabus: Record<string, SubtopicProgress> = {};
  ...
  function saveState(immediate?: boolean) {
    setSyncBusy(true);
    const doSave = () => {
      try {
  ...
  let pendingUndo: { snapshot: AppState; message: string; timeoutId?: any } | null = null;
  ...
  document.querySelectorAll('.tab-btn').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (tabName === 'mocks') drawChart();
      if (tabName === 'plan') renderWeakZones();
    });
  });

  function computeSubjectProgress(subj: SyllabusItem): number {
    let total = 0;
    let done = 0;
    subj.subtopics.forEach((_, i) => {
  ...
  function computeGlobalProgress(): number {
    let total = 0;
    let done = 0;
    SYLLABUS_DATA.forEach((subj) => {
      subj.subtopics.forEach((_, i) => {
  ...
  function updateSubjectCardInPlace(card: HTMLElement, subj: SyllabusItem) {
    const pct = computeSubjectProgress(subj);
    (card.querySelector('.subject-mini-bar__fill') as HTMLElement).style.width = pct + '%';
    card.querySelector('.subject-pct')!.textContent = pct + '%';
  }

  function mockPct(m: MockTest): number {
  ...
  function renderMockList() {
    ...
    const sorted = [...state.mocks].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    sorted.forEach((m) => {
      const row = document.createElement('div');
      ...
      row.innerHTML = `
        ...
        <div class="mock-row__score">
          ...
          <button class="mock-row__del" data-id="${m.id}" aria-label="Delete mock">✕</button>
        </div>
      `;
      row.querySelector('.mock-row__del')!.addEventListener('click', () => {
        pushUndo('DELETED MOCK "' + m.name.slice(0, 24) + '"');
  ...
  (document.getElementById('mock-form') as HTMLFormElement).addEventListener('submit', (e) => {
    e.preventDefault();
    const name = (document.getElementById('mock-name') as HTMLInputElement).value.trim();
    const date = (document.getElementById('mock-date') as HTMLInputElement).value;
    const marks = parseFloat((document.getElementById('mock-marks') as HTMLInputElement).value);
    const total = parseFloat((document.getElementById('mock-total') as HTMLInputElement).value);
    const notes = (document.getElementById('mock-notes') as HTMLInputElement).value.trim();
  ...
  function drawChart() {
    ...
    const sorted = [...state.mocks].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
  ...
  function getWeakestSubjects(n: number) {
  ...
  function renderPlanTarget() {
    ...
    (document.getElementById('daily-target') as HTMLInputElement).value = String(target);
  }

  function subjectTitleById(id: string): string {
  ...
  (document.getElementById('task-form') as HTMLFormElement).addEventListener('submit', (e) => {
    e.preventDefault();
    const subject = (document.getElementById('task-subject') as HTMLSelectElement).value;
    const kind = (document.getElementById('task-kind') as HTMLSelectElement).value;
    const label = (document.getElementById('task-label') as HTMLInputElement).value.trim();
  ...
  function adjustSolved(delta: number) {
  ...
  document.addEventListener('click', (e) => {
    if (!syncMenu.contains(e.target as Node) && e.target !== syncBtn) syncMenu.classList.add('hidden');
  });

  function setSyncBusy(busy: boolean) {
    syncDot.classList.toggle('is-busy', busy);
  }
  function setSyncLabel(text: string) {
  ...
  function openConfirm(title: string, body: string, onOk: () => void) {
  ...
  document.getElementById('sync-reset')!.addEventListener('click', () => {
    ...
    openConfirm(
      'RESET ALL DATA?',
      'This wipes syllabus progress, mock logs and tomorrow\'s plan on this device.',
      () => {
  ...
  /* Standalone PWA Install Fix Logic */
  ...
  function detectPlatformInstructions(): string {
    ...
    return 'Open your browser menu and look for "INSTALL SUMANVOLT" or "ADD TO HOME SCREEN".';
  }
